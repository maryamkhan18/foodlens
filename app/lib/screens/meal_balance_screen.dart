...
class _MealBalanceScreenState extends State<MealBalanceScreen> {
  ...
  Future<void> analyzeMeal() async {
    ...
    try {
      final response = await http.post(
        Uri.parse("http://192.168.100.9:8000/meal-balance"),
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  const Text("Setup your profile first",
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text("Enter your breakfast or current meal",
    ...
    return Column(
      ...
      children: [
        ...
        const Text("Daily Targets",
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        _macroRow("🔥 Calories", targets["calories"], consumed["calories"],
            progress["calories"] ?? 0, Colors.orange),
        _macroRow("💪 Protein", targets["protein"], consumed["protein"],
            progress["protein"] ?? 0, Colors.blue),
        _macroRow("🍚 Carbs", targets["carbs"], consumed["carbs"],
            progress["carbs"] ?? 0, Colors.amber),
        _macroRow("🧈 Fat", targets["fat"], consumed["fat"],
            progress["fat"] ?? 0, Colors.red),
        ...
        const Text("Micronutrients",
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        _macroRow("🌾 Fiber", microsTargets["fiber_g"], microsConsumed["fiber_g"],
            microsTargets["fiber_g"] != null ? ((microsConsumed["fiber_g"] ?? 0) / microsTargets["fiber_g"] * 100).clamp(0, 100) : 0, Colors.green),
        _macroRow("🦴 Calcium", microsTargets["calcium_mg"], microsConsumed["calcium_mg"],
            microsTargets["calcium_mg"] != null ? ((microsConsumed["calcium_mg"] ?? 0) / microsTargets["calcium_mg"] * 100).clamp(0, 100) : 0, Colors.teal),
        _macroRow("🩸 Iron", microsTargets["iron_mg"], microsConsumed["iron_mg"],
            microsTargets["iron_mg"] != null ? ((microsConsumed["iron_mg"] ?? 0) / microsTargets["iron_mg"] * 100).clamp(0, 100) : 0, Colors.brown),
        _macroRow("🍊 Vitamin C", microsTargets["vitc_mg"], microsConsumed["vitc_mg"],
            microsTargets["vitc_mg"] != null ? ((microsConsumed["vitc_mg"] ?? 0) / microsTargets["vitc_mg"] * 100).clamp(0, 100) : 0, Colors.deepOrange),

        const SizedBox(height: 20),

        // ANALYSIS
        const Text("Meal Analysis",
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        Container(
          padding: const EdgeInsets.all(16),
  ...
  Widget _macroRow(String label, dynamic target, dynamic consumed,
      dynamic progress, Color color) {
    double pct = progress is double ? progress : (double.tryParse(progress.toString()) ?? 0);
    return Container(
      ...
      child: Column(
        children: [
          ...
          Align(
            ...
            child: Text("${pct.toStringAsFixed(0)}% complete",
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _field("Height (cm)", _heightController, "e.g. 170", isNumber: true),
